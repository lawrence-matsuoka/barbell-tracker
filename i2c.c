...
void i2c_init(void) {
    ...
    UCB1BR0 = 20;                            // 100kHz @ 1MHz SMCLK
    UCB1BR1 = 0;
    UCB1I2CSA = MPU6050_ADDR;                        // MPU6050 default address
...
void i2c_write_byte(unsigned char addr, unsigned char reg, unsigned char data) {
    ...
    while (UCB1CTL1 & UCTXSTP); // Wait
}

unsigned char i2c_read_byte(unsigned char addr, unsigned char reg) {
    while (UCB1CTL1 & UCTXSTP);           // Wait for stop condition
    UCB1I2CSA = addr;                     // Set slave address BEFORE start

    UCB1CTL1 |= UCTR + UCTXSTT;           // Transmitter mode + START
    while (!(UCB1IFG & UCTXIFG));         // Wait for TX buffer
    UCB1TXBUF = reg;                      // Send register

    while (!(UCB1IFG & UCTXIFG));         // Wait for transmission
    UCB1CTL1 &= ~UCTR;                    // Receiver mode
    UCB1CTL1 |= UCTXSTT;                  // Repeated START

    while (UCB1CTL1 & UCTXSTT);           // Wait for START to complete
    UCB1CTL1 |= UCTXSTP;                  // Send stop

    while (!(UCB1IFG & UCRXIFG));         // Wait for receive complete
    return UCB1RXBUF;
}
...
void i2c_scan() {
    ...
    for (addr; addr < 127; addr++) {
        UCB1I2CSA = addr;
        UCB1CTL1 |= UCTR + UCTXSTT;          // Start + Transmit mode
        while (UCB1CTL1 & UCTXSTT);          // Wait for START to complete
        ...
        UCB1CTL1 |= UCTXSTP;
        while (UCB1CTL1 & UCTXSTP);          // Wait for STOP to complete
    }
